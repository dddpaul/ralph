...
for noun in task doc decision draft; do
  [ -n "$targets" ] && break
  targets=$(extract_quoted "backlog[[:space:]]\{1,\}${noun}[[:space:]]\{1,\}create")
  [ -n "$targets" ] && kind="title"
done
...
if [ -z "$targets" ]; then
  ...
  targets=$(extract_quoted "${edit_prefix}.*[[:space:]]--title")
  if [ -z "$targets" ]; then
    targets=$(extract_quoted "${edit_prefix}.*[[:space:]]-t")
  fi
  [ -n "$targets" ] && kind="title"
fi
...
if [ -z "$targets" ]; then
  targets=$(printf '%s' "$cmd" | sed -n 's/^git checkout -b[[:space:]]*\([^[:space:]]*\).*/\1/p')
  [ -n "$targets" ] && kind="branch"
fi

[ -z "$targets" ] && exit 0

# `sed -n ...p` emits one line per match, and a Bash tool call may carry several
# commands on separate lines — so check each extracted name on its own rather
# than measuring the concatenation.
