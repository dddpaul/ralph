...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 naming-guard.sh replaces grep -qP with portable check (bash [[ ]] or LC_ALL=C grep)
- [ ] #2 notes-guard.sh regex catches --notes at end of command (no value)
- [ ] #3 Inline hook commands in .claude/settings.json updated with same fixes
- [ ] #4 Inline hook commands in skills/ralph-init/templates/settings.json updated with same fixes
- [ ] #5 Smoke test: backlog task create with Cyrillic title is BLOCKED on macOS
- [ ] #6 Smoke test: backlog task edit with bare --notes at end is BLOCKED
- [ ] #7 All other smoke tests from TASK-66 still pass (regression check)
<!-- AC:END -->
